...
import org.noear.solon.serialization.jackson3.xml.impl.NullBeanSerializerModifierImpl;
 ...
 * @since 2.8
 */
public class Jackson3XmlStringSerializer implements EntityStringSerializer {
    public static final String label = "/xml";
    private static final Jackson3XmlStringSerializer _default = new Jackson3XmlStringSerializer();

    /**
     * 默认实例
     */
    public static Jackson3XmlStringSerializer getDefault() {
        return _default;
    }


    private Jackson3XmlDecl<SerializationFeature> serializeConfig;
    private Jackson3XmlDecl<DeserializationFeature> deserializeConfig;

    private AtomicBoolean initStatus = new AtomicBoolean(false);

    public Jackson3XmlStringSerializer(JsonProps jsonProps) {
        loadJsonProps(jsonProps);
    }

    public Jackson3XmlStringSerializer() {
     ...
     * 获取序列化配置
     */
    public Jackson3XmlDecl<SerializationFeature> getSerializeConfig() {
        if (serializeConfig == null) {
            serializeConfig = new Jackson3XmlDecl<>();
        }
     ...
     * 获取反序列化配置
     */
    public Jackson3XmlDecl<DeserializationFeature> getDeserializeConfig() {
        if (deserializeConfig == null) {
            deserializeConfig = new Jackson3XmlDecl<>();
        }
    ...
    protected void loadJsonProps(JsonProps jsonProps) {
        ...
        XmlMapper mapper = getSerializeConfig().getMapper();
